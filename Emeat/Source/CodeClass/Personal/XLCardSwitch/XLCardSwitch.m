...
#pragma mark -
#pragma mark Setter
-(void)setItems:(NSArray<HomePageModel *> *)items {
    _items = items;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ///添加
    if ([self respondsToSelector:@selector(selectCellIndex)]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    card.addShoppingCardBtn.tag = indexPath.row;
    [card.addShoppingCardBtn addTarget:self action:@selector(addShoppingCardBtnAction:) forControlEvents:1];
...
-(void)addShoppingCardBtnAction:(UIButton*)btn{
    if ([self respondsToSelector:@selector(selectCardAddShoppingIndex)]) {
        self.selectCardAddShoppingIndex(btn.tag);
    }
    //DLog(@"tttc== %ld" ,btn.tag);
}
