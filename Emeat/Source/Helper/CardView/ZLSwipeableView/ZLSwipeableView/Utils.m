...
ZLSwipeableViewDirection ZLSwipeableViewDirectionFromVector(CGVector directionVector) {
    ...
    if (ABS(directionVector.dx) > ABS(directionVector.dy)) {
        ...
        if (directionVector.dy > 0) {
            ...
            //创建一个消息对象  //此处做了处理
            NSNotification * notices = [NSNotification notificationWithName:@"down" object:nil userInfo:@{@"key":@"down"}];
            //发送消息
            ...
            //if (directionVector.dy < 0) {
                //创建一个消息对象  //此处做了处理
                NSNotification * notices = [NSNotification notificationWithName:@"up" object:nil userInfo:@{@"key":@"up"}];
                //发送消息
                [[NSNotificationCenter defaultCenter]postNotification:notices];
