...
@property (weak, nonatomic) MyViewController1 *viewController;
...
- (void)setCloseButtonType:(ButtonPositionType)closeButtonType {
    ...
    if (closeButtonType == ButtonPositionTypeNone) {
        ...
        }else if (closeButtonType == ButtonPositionTypeBottomMid){
              closeFrame.origin.x = ((CGRectGetWidth(self.containerView.frame) - CGRectGetWidth(closeFrame))/2);
            closeFrame.origin.y = MaxY(self.containerView)-120*kScale;
        }
        else {
...
- (void)showWithPresentView:(UIView *)presentView animated:(BOOL)animated {
    ...
    MyViewController1 *viewController = [[MyViewController1 alloc] init];
...
- (instancetype)init {
    ...
    self = [super initWithFrame:(CGRect){0, 0, 34*kScale, 34*kScale }];
    if (self) {
        
        [self setBackgroundImage:[UIImage imageNamed:@"cancel_Version"] forState:UIControlStateNormal];
